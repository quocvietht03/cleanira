...
function cleanira_woocommerce_item_sold($product_id)
{
  ...
  if ($total_quantity_sold > 1) {
    echo esc_html__(' items sold', 'cleanira');
  } else {
    echo esc_html__(' item sold', 'cleanira');
  }
  echo '</div>';
}
...
function cleanira_get_free_shipping()
{
  ...
  $currency_symbol = get_woocommerce_currency_symbol();
  if(cleanira_is_appointment_in_cart()){
...
?>
  <div class="bt-product-icon-btn">
    <a class="bt-icon-btn bt-product-compare-btn <?php echo esc_attr(cleanira_is_compare($product->get_id()) ? 'added' : 'no-added'); ?>" href="#" data-id="<?php echo esc_attr($product->get_id()); ?>">
    ...
    <a class="bt-icon-btn bt-product-wishlist-btn  <?php echo esc_attr(cleanira_is_wishlist($product->get_id()) ? 'added' : 'no-added'); ?>" href="#" data-id="<?php echo esc_attr($product->get_id()); ?>">
...
function cleanira_custom_add_to_cart_text($text, $product)
{
  if ($product->is_type('simple')) {
    ...
    $text = sprintf(__('Add to cart - %s', 'cleanira'), $formatted_price);
  }
  return $text;
}
...
function cleanira_free_shipping_appointment($rates, $package)
{
  if (!function_exists('get_field')) {
    return $rates;
  }
  ...
  if ($has_appointment) {
    foreach ($rates as $rate_id => $rate) {
      ...
      $rates[$rate_id]->label = __('Free Shipping', 'cleanira');
...
add_filter('woocommerce_package_rates', 'cleanira_free_shipping_appointment', 10, 2);

function cleanira_is_appointment_in_cart()
{
  if (!function_exists('get_field')) {
    return false;
  }
...
add_action('wp_ajax_cleanira_remove_section', 'cleanira_remove_section');
add_action('wp_ajax_nopriv_cleanira_remove_section', 'cleanira_remove_section');

function cleanira_remove_section()
{
  session_start();
