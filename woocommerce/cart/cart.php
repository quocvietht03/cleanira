...
<div class="bt-cart-content" data-coupon="<?php echo esc_attr($coupon); ?>">
	...
	if ($cart_total < $free_shipping_threshold) {
		...
		$percentage = 100;
		$message = __('<p class="bt-congratulation"> Congratulations! You have free shipping!</p>', 'cleanira');
	}
	if ($time_promotion && $time_promotion['promotion'] === true) {
		...
		if (!empty($promotion_time)) {
			echo '<div class="bt-time-promotion"><span class="bt-icon">🔥</span>' . sprintf(
				__('Your cart will expire in <span id="countdown" data-time="%s">%s</span> minutes! Please checkout now before your items sell out!', 'cleanira'),
				$promotion_time,
				$promotion_time
			) . '</div>';
		}
		...
		<table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
			<thead>
				<tr>
					...
					<th class="product-thumbnail"><?php esc_html_e('Products', 'cleanira'); ?></th>
					<th class="product-name"></th>
					<th class="product-price"><?php esc_html_e('Price', 'cleanira'); ?></th>
					<th class="product-quantity"><?php esc_html_e('Quantity', 'cleanira'); ?></th>
					<th class="product-subtotal"><?php esc_html_e('Total Price', 'cleanira'); ?></th>
					<th class="product-remove"><span class="screen-reader-text"><?php esc_html_e('Remove item', 'cleanira'); ?></span></th>
				</tr>
			</thead>
			<tbody>
				...
				?>
						<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr(apply_filters('woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key)); ?>">
							...
							<td class="product-name" data-title="<?php esc_attr_e('Product', 'cleanira'); ?>">
								<?php
								if (! $product_permalink) {
								...
								if ($_product->backorders_require_notification() && $_product->is_on_backorder($cart_item['quantity'])) {
									echo wp_kses_post(apply_filters('woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__('Available on backorder', 'cleanira') . '</p>', $product_id));
								}
								?>
							</td>

							<td class="product-price" data-title="<?php esc_attr_e('Price', 'cleanira'); ?>">
								<?php
								echo apply_filters('woocommerce_cart_item_price', WC()->cart->get_product_price($_product), $cart_item, $cart_item_key); // PHPCS: XSS ok.
								?>
							</td>

							<td class="product-quantity" data-title="<?php esc_attr_e('Quantity', 'cleanira'); ?>">
								<?php
							...
							<td class="product-subtotal" data-title="<?php esc_attr_e('Subtotal', 'cleanira'); ?>">
								<?php
  ...
  <path d="M9.64052 9.10965C9.67536 9.14449 9.703 9.18586 9.72186 9.23138C9.74071 9.2769 9.75042 9.32569 9.75042 9.37496C9.75042 9.42424 9.74071 9.47303 9.72186 9.51855C9.703 9.56407 9.67536 9.60544 9.64052 9.64028C9.60568 9.67512 9.56432 9.70276 9.51879 9.72161C9.47327 9.74047 9.42448 9.75017 9.37521 9.75017C9.32594 9.75017 9.27714 9.74047 9.23162 9.72161C9.1861 9.70276 9.14474 9.67512 9.1099 9.64028L6.00021 6.53012L2.89052 9.64028C2.82016 9.71064 2.72472 9.75017 2.62521 9.75017C2.5257 9.75017 2.43026 9.71064 2.3599 9.64028C2.28953 9.56991 2.25 9.47448 2.25 9.37496C2.25 9.27545 2.28953 9.18002 2.3599 9.10965L5.47005 5.99996L2.3599 2.89028C2.28953 2.81991 2.25 2.72448 2.25 2.62496C2.25 2.52545 2.28953 2.43002 2.3599 2.35965C2.43026 2.28929 2.5257 2.24976 2.62521 2.24976C2.72472 2.24976 2.82016 2.28929 2.89052 2.35965L6.00021 5.46981L9.1099 2.35965C9.18026 2.28929 9.2757 2.24976 9.37521 2.24976C9.47472 2.24976 9.57016 2.28929 9.64052 2.35965C9.71089 2.43002 9.75042 2.52545 9.75042 2.62496C9.75042 2.72448 9.71089 2.81991 9.64052 2.89028L6.53036 5.99996L9.64052 9.10965Z" fill="#212121"/>
										...
										esc_attr(sprintf(__('Remove %s from cart', 'cleanira'), wp_strip_all_tags($product_name))),
										esc_attr($product_id),
				...
				<tr>
					<td colspan="6" class="actions">
						...
						<?php if (wc_coupons_enabled()) { ?>
							<div class="coupon">
								<label for="coupon_code" class="screen-reader-text"><?php esc_html_e('Coupon:', 'cleanira'); ?></label> <input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e('Add voucher discount', 'cleanira'); ?>" /> <button type="submit" class="button<?php echo esc_attr(wc_wp_theme_get_element_class_name('button') ? ' ' . wc_wp_theme_get_element_class_name('button') : ''); ?>" name="apply_coupon" value="<?php esc_attr_e('Apply Code', 'cleanira'); ?>"><?php esc_html_e('Apply Code', 'cleanira'); ?></button>
								<?php do_action('woocommerce_cart_coupon'); ?>
							</div>
						<?php } ?>

						<button type="submit" class="button<?php echo esc_attr(wc_wp_theme_get_element_class_name('button') ? ' ' . wc_wp_theme_get_element_class_name('button') : ''); ?>" name="update_cart" value="<?php esc_attr_e('Update cart', 'cleanira'); ?>"><?php esc_html_e('Update cart', 'cleanira'); ?></button>
...
<div class="cart-collaterals <?php echo esc_attr(cleanira_is_appointment_in_cart() ? 'is_appointment' : ''); ?>">
	<?php
	/**
...
<?php do_action('cleanira_woocommerce_template_cross_sell') ?>
