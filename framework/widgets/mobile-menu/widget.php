...
namespace CleaniraElementorWidgets\Widgets\MobileMenu;
...
class Widget_MobileMenu extends Widget_Base
{
	...
	{
		return __('Mobile Menu', 'cleanira');
	}
	...
	protected function register_content_section_controls()
	{
		...
		if ( ! empty( $menus ) ) {
			$this->add_control(
				'menu',
				[
					'label' => esc_html__( 'Menu', 'cleanira' ),
					...
					'description' => sprintf(
						...
						esc_html__( 'Go to the %1$sMenus screen%2$s to manage your menus.', 'cleanira' ),
			...
			$this->add_control(
				...
				[
					...
					'alert_type' => 'info',
					'heading' => esc_html__( 'There are no menus in your site.', 'cleanira' ),
					'content' => sprintf(
						/* translators: 1: Link opening tag, 2: Link closing tag. */
						esc_html__( 'Go to the %1$sMenus screen%2$s to create one.', 'cleanira' ),
	...
	protected function register_style_content_section_controls()
	{
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__('Main Menu', 'cleanira'),
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Text Color', 'cleanira' ),
		...
		$this->start_controls_tab(
			...
			[
				'label' => esc_html__( 'Active', 'cleanira' ),
			]
		);
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Vertical Padding', 'cleanira' ),
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Toggle Offset', 'cleanira' ),
				'type' => Controls_Manager::SLIDER,
				'size_units' => [ 'px', 'em', 'rem', 'custom' ],
				'range' => [
					'px' => [
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__('Sub Menu', 'cleanira'),
				'tab' => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__('Additional Options', 'cleanira'),
				'tab' => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->add_control(
			'heading_toggle_menu',
			[
				'label' => esc_html__( 'Toggle Menu', 'cleanira' ),
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Divider Menu', 'cleanira' ),
				'type' => Controls_Manager::HEADING,
				'separator' => 'before',
			]
		);
		...
		$this->add_control(
			'divider_menu_style',
			[
				'label' => esc_html__( 'Style', 'cleanira' ),
				'type' => Controls_Manager::SELECT,
				'options' => [
					'none' => esc_html__( 'None', 'cleanira' ),
					'solid' => esc_html__( 'Solid', 'cleanira' ),
					'double' => esc_html__( 'Double', 'cleanira' ),
					'dotted' => esc_html__( 'Dotted', 'cleanira' ),
					'dashed' => esc_html__( 'Dashed', 'cleanira' ),
				],
				'default' => 'solid',
				'selectors' => [
		...
		$this->add_control(
			'divider_menu_weight',
			[
				'label' => esc_html__( 'Width', 'cleanira' ),
				'type' => Controls_Manager::SLIDER,
